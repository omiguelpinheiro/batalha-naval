...
navios = {0: "porta_aviao", 1: "navio_tanque", 2: "contratorpedeiro", 3: "submarino"}
...
def inicia_partida():
    ...
    nome_jogador_1 = cli.pede_dado(0, len(mensagem_inserir_jogador_1), 25, console).decode()
    cli.escreve_na_tela(0, len(mensagem_inserir_jogador_1) + len(nome_jogador_1), " -> Inserido", console)
    cli.escreve_na_tela(1, 0, mensagem_inserir_jogador_2, console)
    nome_jogador_2 = cli.pede_dado(1, len(mensagem_inserir_jogador_2), 25, console).decode()
    cli.escreve_na_tela(1, len(mensagem_inserir_jogador_2) + len(nome_jogador_2), " -> Inserido", console)
    ...
    for id_jogador in id_jogadores:
        while True:
            ...
            cli.escreve_na_tela(0, 28, f"Num      Embarcações            Ocupa     Qtd", console)
            cli.escreve_na_tela(1, 28, f" 0       Porta-aviões       5 Quadrados    {jog['navios'][0]}", console)
            cli.escreve_na_tela(2, 28, f" 1       Navio-tanque       4 Quadrados    {jog['navios'][1]}", console)
            cli.escreve_na_tela(3, 28, f" 2     Contratorpedeiro     3 Quadrados    {jog['navios'][2]}", console)
            cli.escreve_na_tela(4, 28, f" 3        Submarino         2 Quadrados    {jog['navios'][3]}", console)
            ...
            navio = cli.pede_dado(6, 28 + len(mensagem_selecao_embarcacao), 1, console).decode()
            # Apaga mensagem e prompt acima
            cli.escreve_na_tela(6, 28, " " * (len(mensagem_selecao_embarcacao) + 3), console)
            ...
            orientacao = cli.pede_dado(6, 28 + len(mensagem_orientacao), 1, console).decode().upper()
            # Apaga mensagem e prompt acima
            cli.escreve_na_tela(6, 28, " " * (len(mensagem_orientacao) + 3), console)
            cli.escreve_na_tela(7, 28, " " * (len(mensagem_explicacao_orientacao) + 3), console)
            ...
            primeiro_quadrado = cli.pede_dado(6, 28 + len(mensagem_primeiro_quadrado), 3, console).decode().upper()
            # Apaga mensagem e prompt cima
            cli.escreve_na_tela(6, 28, " " * (len(mensagem_primeiro_quadrado) + 3), console)
            cli.escreve_na_tela(7, 28, " " * (len(mensagem_exemplo_entrada) + 3), console)

            retorno = jogador.posiciona_navio(int(navio), primeiro_quadrado, orientacao, id_jogador)
            if retorno == -1:
                ...
                cli.escreve_na_tela(8, 28, mensagem_erro_falta_espaco_tabuleiro, console)
            elif retorno == -7:
                cli.escreve_na_tela(8, 28, mensagem_erro_sobreposicao_barcos, console)
            else:
                cli.escreve_na_tela(8, 28, " " * (len(mensagem_erro_barco_falta) + 3), console)
    ...
    while True:
        ...
        cli.desenha_mapa(jogador.consulta_jogador(0), jogador.consulta_jogador(1))
        cli.escreve_na_tela(13, 0, f"Jogador {id_jogadores[0]} | Qual quadrante deseja atacar? (Ex. A-8, h-2, C4 e d7): ", console)
        coordenada = cli.pede_dado(13, len(mensagem_pede_ataque), 3, console).decode()
        retorno = jogador.ataca_jogador(id_jogadores[0], id_jogadores[1], coordenada)
        cli.escreve_na_tela(17, 0, mensagem_erro_quadrado, console)
        if retorno == -1:
            cli.escreve_na_tela(14, 0, mensagem_erro_quadrado, console)
        if retorno == -2:
            cli.escreve_na_tela(14, 0, mensagem_erro_quadrado_ja_atacado, console)
        if retorno == 1:
            cli.desenha_mapa(jogador.consulta_jogador(0), jogador.consulta_jogador(1))
            cli.escreve_na_tela(13, 0, f"{atacante['nome']} venceu o jogo! Parabéns!", console)
